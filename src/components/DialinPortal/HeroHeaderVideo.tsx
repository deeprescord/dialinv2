...
export const HeroHeaderVideo = React.forwardRef<HeroHeaderVideoHandle, HeroHeaderVideoProps>(({ 
  ...
  const [isVideoMuted, setIsVideoMuted] = useState(false); // Start unmuted for better UX
  ...
  useEffect(() => {
    if (show360) {
      ...
      // Ensure both HTML5 videos are muted; 360 will autoplay muted per policy
      if (videoRef.current) videoRef.current.muted = true;
      if (bgVideoRef.current) bgVideoRef.current.muted = true;
      setIsPlaying(true);
      setIsVideoMuted(true); // start muted to satisfy autoplay policies
      setVideoVolume(volume ?? 0.7);
    }
  }, [show360, volume]);
